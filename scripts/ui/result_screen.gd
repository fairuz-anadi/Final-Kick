...
func show_result() -> void:
	...
	# The run's total goes to the leaderboard after EVERY clear, not just the
	# ending — a player who stops after level 1 still competes for the top 5.
	# Leaderboard keeps one entry per name with the highest score, so each
	# later clear simply raises the same entry.
